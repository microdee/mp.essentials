...
namespace VVVV.Nodes
{
    ...
    //class definition
    public unsafe class BezierSpline : IPlugin, IDisposable
    {
        ...
        private IValueIn FTandist;

        //output pin declaration
        private IValueOut FOutput;
        private IValueOut FTangent;
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            if (!FDisposed)
            {
                ...
                FHost.Log(TLogType.Debug, "BezierSpline is being deleted");
        ...
        public static IPluginInfo PluginInfo
        {
            get
            {
                if (FPluginInfo == null)
                {
                    ...
                    FPluginInfo.Name = "BezierSpline";
                    //the nodes category: try to use an existing one
                    FPluginInfo.Category = "Value";
                    //the nodes version: optional. leave blank if not
                    //needed to distinguish two nodes of the same name and category
                    FPluginInfo.Version = "Tangent";

                    //the nodes author: your sign
                    FPluginInfo.Author = "woei, microdee";
        ...
        public void SetPluginHost(IPluginHost Host)
        {
            ...
            FHost.CreateValueInput("Tangent Length", 1, null, TSliceMode.Dynamic, TPinVisibility.Hidden, out FTandist);
            FTandist.SetSubType(double.MinValue, double.MaxValue, 0.01, 0.1, false, false, false);
            ...
            FHost.CreateValueOutput("Tangent", 1, null, TSliceMode.Dynamic, TPinVisibility.True, out FTangent);
            FTangent.SetSubType(double.MinValue, double.MaxValue, 0.01, 0, false, false, false);
        ...
        public unsafe void Evaluate(int SpreadMax)
        {
                ...
                FTandist.PinIsChanged)
            {
                ...
                double[] returnTans = new double[0];
                int iIncr = 0, pIncr = 0;
                for (int c = 0; c < maxLoop; c++)
                {
                    ...
                    Array.Resize(ref returnTans, returnTans.Length + (iCount * vecSize));
                    ...
                    for (int v = 0; v < vecSize; v++)
                    {
                        ...
                        FTandist.GetValue(c, out var tandist);
                        var ptres = PolyBezierDC(inArr, ptArr, ctrlArr, isClosed, tandist);

                        for (int o = 0; o < ptres.p.Length; o++)
                        {
                            returnArr[(iIncr + o) * vecSize + v] = ptres.p[o];
                            returnTans[(iIncr + o) * vecSize + v] = ptres.t[o];
                        }
                    }
                ...
                double* outVal, outTan;
                FOutput.SliceCount = returnArr.Length;
                FOutput.GetValuePointer(out outVal);
                FTangent.SliceCount = returnArr.Length;
                FTangent.GetValuePointer(out outTan);
                for (int i = 0; i < returnArr.Length; i++)
                {
                    outVal[i] = returnArr[i];
                    outTan[i] = returnTans[i];
                }
            }
        }
        ...
        private (double[] p, double[] t) PolyBezierDC(double[] input, double[] pts, double[] ctrls, bool IsClosed, double tandist)
        {
            double[] output = new double[input.Length];
            double[] tangents = new double[input.Length];
            int cCount = pts.Length - 1;
            if (IsClosed)
                cCount++;
            for (int i = 0; i < input.Length; i++)
            {
                ...
                var muu = mu - cId;
                var ptan = BezierCurveDC(pt1, pt2, c1, c2, muu, tandist);
